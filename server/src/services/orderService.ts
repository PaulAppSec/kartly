...
export const orderService = {
  ...
  // Object-level authz — a customer may only read their own order; others get
  // 404 (secure baseline for IDOR #5).
  async getById(userId: string, orderId: string) {
    const order = await orderRepo.findById(orderId);
    if (!order || order.customerId !== userId) throw new HttpError(404, "Order not found.");
    return toDTO(order as OrderRow);
  },

  // Secure checkout: price is computed server-side from the DB, stock is
  // enforced, and the coupon is applied exactly once via an atomic conditional
  // update — the whole thing runs in a transaction. This is the correct
  // baseline for the business-logic lesson (#22); Phase 3 breaks each guard.
